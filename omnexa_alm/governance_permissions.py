...
def policy_query_conditions(user=None):
	...
	return f"(`tabALM Policy Version`.maker = {esc} or `tabALM Policy Version`.checker = {esc} or `tabALM Policy Version`.rejector = {esc})"
...
def snapshot_query_conditions(user=None):
	...
	if _is_privileged(user):
		return ""
	esc = frappe.db.escape(user)
	return f"`tabALM Audit Snapshot`.actor = {esc}"
