...
def execute(filters=None):
	...
	pol_total = frappe.db.count("ALM Policy Version")
	pending = frappe.db.count("ALM Policy Version", {{"status": "PENDING_APPROVAL"}})
	approved = frappe.db.count("ALM Policy Version", {{"status": "APPROVED"}})
	rejected = frappe.db.count("ALM Policy Version", {{"status": "REJECTED"}})
	snaps = frappe.db.count("ALM Audit Snapshot")
