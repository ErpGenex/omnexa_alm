...
from omnexa_alm.governance import (
...
class TestAlmGovernance(FrappeTestCase):
	def test_submit_policy_and_snapshot(self):
		...
		pols = list_policy_versions("omnexa_alm", policy_name="pricing_policy")
		...
		snaps = list_audit_snapshots("omnexa_alm", process_name="decision_preview", limit=10)
		self.assertTrue(snaps)

		ov = governance_overview("omnexa_alm")
